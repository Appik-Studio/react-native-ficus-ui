...
import { createShadowStyles, createPositionStyle, createSpacingStyles, createBorderWidthStyles, createBorderColorStyles, createBorderRadiusStyles, getThemeColor, createAvatarStyles, getThemeProperty, getFontWeight, getThemeFontFamily } from '../../theme/theme.service';
...
export const getStyle = (theme, props) => {
  ...
  computedStyle.avatarImage = {
    ...
    backgroundColor: props.bg ? getThemeColor(theme.colors, props.bg) : props.colorScheme ? getThemeColor(theme.colors, `${props.colorScheme}.300`) : props.name ? lightenColor(stringToColor(props.name), 30) : getThemeColor(theme.colors, 'gray.300')
  };
  computedStyle.text = {
    ...
    color: props.color ? getThemeColor(theme.colors, props.color) : props.colorScheme ? getThemeColor(theme.colors, `${props.colorScheme}.800`) : props.name ? darkenColor(stringToColor(props.name), 40) : getThemeColor(theme.colors, 'gray.800'),
    fontSize: props.fontSize ? getThemeProperty(theme.fontSize, props.fontSize) : getThemeProperty(theme.avatar, props.size) / 2,
