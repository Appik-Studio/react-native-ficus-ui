...
import { validateHTMLColorRgb, validateHTMLColorSpecialName, validateHTMLColorHex, validateHTMLColorHsl, validateHTMLColorName } from 'validate-color';
...
export const isInputEvent = value => value && isObject(value) && isObject(value.target);
...
export const isValidColor = color => {
  return validateHTMLColorRgb(color) || validateHTMLColorSpecialName(color) || validateHTMLColorHex(color) || validateHTMLColorHsl(color) || validateHTMLColorName(color);
};
export const getSpecificProps = (obj, ...keys) =>
//@ts-ignore
keys.reduce((a, c) => ({
  ...a,
  [c]: obj[c]
}), {});
export const removeSpecificProps = (obj, ...keys) => keys.reduce((a, c) => {
  //@ts-ignore
  delete a[c];
  return a;
}, obj);
